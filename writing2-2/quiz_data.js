const quizData = [
    {
        text: "Problem 1: 부등식 $\\frac{2x-1}{3} < \\frac{x+2}{2} \\le \\frac{3x+1}{4}$ 를 푸는 과정을 서술하시오.",
        steps: [
            { 
                q: "1단계: 분수 부등식을 풀기 위해 각 부등식을 연립으로 나누어 설정하면?", 
                options: [
                    {t: "$\\frac{2x-1}{3} < \\frac{x+2}{2}$ 와 $\\frac{x+2}{2} \\le \\frac{3x+1}{4}$ 를 각각 풀어야 한다.", c: true}, 
                    {t: "세 분수를 통분하여 하나의 부등식으로 합친 뒤 한 번에 풀어야 한다.", c: false}
                ] 
            },
            { 
                q: "2단계: 첫 번째 부등식 $\\frac{2x-1}{3} < \\frac{x+2}{2}$ 를 정리하면?", 
                options: [
            ...
            { 
                q: "3단계: 두 번째 부등식 $\\frac{x+2}{2} \\le \\frac{3x+1}{4}$ 를 정리하면?", 
                options: [
    ...
    },
    {
        ...
        steps: [
            { 
                ...
                options: [
                    {t: "$2x-1=0$ 에서 $x=\\frac{1}{2}$, $x+2=0$ 에서 $x=-2$ 이므로 $x < -2$, $-2 \\le x < \\frac{1}{2}$, $x \\ge \\frac{1}{2}$ 으로 나눈다.", c: true}, 
                    {t: "$2x-1=0$ 에서 $x=\\frac{1}{2}$, $x+2=0$ 에서 $x=-2$ 이므로 $x < -2$, $x \\ge \\frac{1}{2}$ 으로만 나눈다.", c: false}
                ] 
            },
            { 
            ...
            { 
                q: "4단계: [경우 2] $-2 \\le x < \\frac{1}{2}$ 일 때, 두 절댓값의 부호를 판단하여 식을 세우면?", 
                options: [
                    {t: "2) $-2 \\le x < \\frac{1}{2}$ 일 때, $2x-1 < 0$, $x+2 \\ge 0$ 이므로 $-(2x-1) - (x+2) \\le 3$ 이 된다.", c: true}, 
                    {t: "2) $-2 \\le x < \\frac{1}{2}$ 일 때, $2x-1 > 0$, $x+2 \\ge 0$ 이므로 $(2x-1) - (x+2) \\le 3$ 이 된다.", c: false}
                ] 
            },
            { 
                q: "5단계: [경우 2]의 부등식을 정리하여 해당 구간 내의 해를 확정하면?", 
                options: [
                    {t: "$-3x-1 \\le 3$ 에서 $x \\ge -\\frac{4}{3}$ 이므로, 구간 조건과 교차하여 해는 $-\\frac{4}{3} \\le x < \\frac{1}{2}$ 이다.", c: true}, 
                    {t: "$-3x-1 \\le 3$ 에서 $x \\ge -\\frac{4}{3}$ 이므로, 해는 $-2 \\le x < \\frac{1}{2}$ 이다.", c: false}
                ] 
            },
            { 
                q: "6단계: [경우 3] $x \\ge \\frac{1}{2}$ 일 때, 두 절댓값의 부호를 판단하여 식을 세우면?", 
                options: [
                    {t: "3) $x \\ge \\frac{1}{2}$ 일 때, $2x-1 \\ge 0$, $x+2 > 0$ 이므로 $(2x-1) - (x+2) \\le 3$ 이 된다.", c: true}, 
                    {t: "3) $x \\ge \\frac{1}{2}$ 일 때, 둘 다 양수이므로 $-(2x-1) - (x+2) \\le 3$ 이 된다.", c: false}
                ] 
            },
            { 
                q: "7단계: [경우 3]의 부등식을 정리하여 해당 구간 내의 해를 확정하면?", 
                options: [
                    {t: "$x-3 \\le 3$ 에서 $x \\le 6$ 이므로, 구간 조건과 교차하여 해는 $\\frac{1}{2} \\le x \\le 6$ 이다.", c: true}, 
            ...
            { 
                ...
                options: [
                    {t: "경우 1의 해는 없고, 경우 2와 경우 3의 해가 이어지므로 최종 해는 $-\\frac{4}{3} \\le x \\le 6$ 이다.", c: true}, 
    ...
    },
    {
        ...
        steps: [
            ...
            { 
                ...
                options: [
                    {t: "$1 \\le x \\le 4$ 와 $(x < -1$ 또는 $x > 4)$ 의 공통 범위는 없으므로, 해는 존재하지 않는다.", c: true}, 
                    {t: "$1 \\le x \\le 4$ 와 $(x < -1$ 또는 $x > 4)$ 의 공통 범위는 $1 \\le x < 4$ 이다.", c: false}
                ] 
            },
            { 
                q: "4단계: 위 결과를 바탕으로 해에 속하는 정수의 개수를 최종 서술하면?", 
                options: [
                    {t: "따라서 연립부등식의 해가 존재하지 않으므로 정수의 개수도 $0$ 개이다.", c: true}, 
                    {t: "따라서 연립부등식의 해에 속하는 정수는 $1, 2, 3$ 이므로 $3$ 개이다.", c: false}
                ] 
            }
        ]
    }
